...
const createRow = async (values) => {
  const response = await hubspotClient.cms.hubdb.rowsApi.createTableRow(dbTableId, { values });
...
exports.main = async (event) => {
  ...
  const details = typeof event.inputFields['details'] === 'undefined' ? '' : event.inputFields['details'];
  const date = typeof event.inputFields['date'] === 'undefined' ? Date.now() : event.inputFields['date'];
  const speakers = typeof event.inputFields['speakers'] === 'undefined' ? '' : event.inputFields['speakers'];
  const address = typeof event.inputFields['address'] === 'undefined' ? '' : event.inputFields['address'];
